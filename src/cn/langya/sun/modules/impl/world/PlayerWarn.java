        ...
        if (mc.player.ticksExisted % 6 == 0) {
            for (final Entity ent : mc.world.loadedEntityList) {
                if (ent instanceof EntityPlayer && ent != mc.player) {
                    ...
                    if (HYTUtils.isStrength(player) > 0 && !PlayerWarn.flaggedEntity.contains(player) && !Teams.isSameTeam(player)) {
                        PlayerWarn.flaggedEntity.add(player);
                        NotificationManager.add("有新的傻逼", player.getName() + " 是傻逼力量狗，快去斩杀他的亲妈吧", TrayIcon.MessageType.WARNING);
                    }
                    if (HYTUtils.isRegen(player) > 0 && !PlayerWarn.flaggedEntity.contains(player) && !Teams.isSameTeam(player)) {
                        PlayerWarn.flaggedEntity.add(player);
                        NotificationManager.add("有新的傻逼", player.getName() + " 是傻逼生命恢复狗，快去斩杀他的亲妈吧", TrayIcon.MessageType.WARNING);
                    }
                    if (HYTUtils.isHoldingGodAxe(player) && !PlayerWarn.flaggedEntity.contains(player) && !Teams.isSameTeam(player)) {
                        PlayerWarn.flaggedEntity.add(player);
                        NotificationManager.add("有新的傻逼", player.getName() + " 正在使用秒人斧击杀他的母亲，主播小心点别被杀了", TrayIcon.MessageType.WARNING);
                    }
                    if (HYTUtils.isKBBall(player.getHeldItem(EnumHand.MAIN_HAND)) && !PlayerWarn.flaggedEntity.contains(player) && !Teams.isSameTeam(player)) {
                        PlayerWarn.flaggedEntity.add(player);
                        NotificationManager.add("有新的傻逼", player.getName() + " 正在使用击退球把他的老母打到十里开外，主播小心点", TrayIcon.MessageType.WARNING);
                    }
                    if (HYTUtils.hasEatenGoldenApple(player) <= 0 || PlayerWarn.flaggedEntity.contains(player) || Teams.isSameTeam(player)) {
                        continue;
                    }
                    PlayerWarn.flaggedEntity.add(player);
