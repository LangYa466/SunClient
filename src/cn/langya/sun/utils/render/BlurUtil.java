...
public class BlurUtil {
    ...
    private static void setValues(double strength, double x, double y, double w, double h, double width, double height) {
        ...
        lastH = h;
        for (int i = 0; i < 2; ++i) {
            shaderGroup.listShaders.get(i).getShaderManager().getShaderUniform("Radius").set((float)strength);
            shaderGroup.listShaders.get(i).getShaderManager().getShaderUniform("BlurXY").set((float)x, (float)(height - y - h));
            shaderGroup.listShaders.get(i).getShaderManager().getShaderUniform("BlurCoord").set((float)w, (float)h);
        }
    }
    ...
    public static void blurArea(float x, float y, float x2, float y2, float blurStrength) {
        ...
        shaderGroup.loadShaderGroup(mc.timer.field_194148_c);
    ...
    static {
        lastStrength = 5.0;
        blurShader = new ResourceLocation("sunclient/shader/blur.json");
    }
}
