...
class ClickGui : GuiScreen() {
    ...
    override fun initGui() {
        ...
        val moduleCategories = modules.groupBy { it.category }  // 根据分类进行分组
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        for ((module, position) in modulePositions) {
            ...
            RenderUtils.drawRoundedRect(position.first - 2, position.second - 2, fontRendererObj.getStringWidth("${module.name}: ${if (module.state) "Enabled" else "Disabled"}") + 4, fontRendererObj.FONT_HEIGHT + 4, 5, Color(0, 0, 0, 150))
            drawString(mc.fontRendererObj, "$category > ${module.name}: ${if (module.state) "Enabled" else "Disabled"}", position.first, position.second, -1)
        }
